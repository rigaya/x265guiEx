...
template <BOOL aligned_store>
static __forceinline void convert_lw48_to_nv12_16bit_simd(void *pixel, CONVERT_CF_DATA *pixel_data, const int width, const int height) {
	__m128i x0, x1, x2, x3;
	...
	for (int y = 0; y < height; y += 2, ycp_line += width*2, y_line += width*2, c_line += width) {
		...
		for ( ; dst_y < dst_y_fin; ycp += 48, ycp_w += 48, dst_y += 8, dst_c += 8) {
			...
			_mm_store_switch_si128((__m128i*)dst_y, x1);
			...
			_mm_store_switch_si128((__m128i*)(dst_y + width), x1);
		
			x0 = _mm_avg_epu16(x0, x2);
		
			_mm_store_switch_si128((__m128i*)dst_c, x0);
		}
	}
}
	...
	for (int y = 0; y < height; y += 4) {
		for (int i = 0; i < 2; i++) {
			...
			for ( ; dst_y < dst_y_fin; ycp += 48, ycp_w += 48, dst_y += 8, dst_c += 8) {
				...
				_mm_store_switch_si128((__m128i*)(dst_y + width*2), x1);
				...
				x1 = _mm_packus_epi32_simd(x1, x0);
...
void convert_lw48_to_nv16_16bit_simd(void *pixel, CONVERT_CF_DATA *pixel_data, const int width, const int height) {
	...
	for (; ycp < ycp_fin; ycp += 48, dst_Y += 8, dst_C += 8) {
		...
		_mm_store_switch_si128((__m128i *)dst_Y, x1);
		_mm_store_switch_si128((__m128i *)dst_C, x2);
	}
}
template <BOOL aligned_store>
void convert_lw48_to_yuv444_simd(void *pixel, CONVERT_CF_DATA *pixel_data, const int width, const int height) {
	...
	for ( ; ycp < ycp_fin; ycp += 96, dst_y += 16, dst_u += 16, dst_v += 16) {
		...
		_mm_store_switch_si128((__m128i*)dst_y, xY);
		_mm_store_switch_si128((__m128i*)dst_u, xU);
		_mm_store_switch_si128((__m128i*)dst_v, xV);
	}
}
template <BOOL aligned_store>
void convert_lw48_to_yuv444_16bit_simd(void *pixel, CONVERT_CF_DATA *pixel_data, const int width, const int height) {
	...
	for ( ; ycp < ycp_fin; ycp += 48, dst_y += 8, dst_u += 8, dst_v += 8) {
		...
		gather_y_u_v_from_yc48(x1, x2, x3);
		_mm_store_switch_si128((__m128i*)dst_y, x1);
		_mm_store_switch_si128((__m128i*)dst_u, x2);
