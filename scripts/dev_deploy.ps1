...
<#
.SYNOPSIS
  x265guiEx をビルドし、AviUtl2 Plugin へ配置して起動する。
...
$Sln = Join-Path $RepoRoot "x265guiEx.sln"
...
if ($Platform -eq "x64") {
    $srcAuo2 = Join-Path $RepoRoot "$Configuration\x64\x265guiEx.auo2"
} else {
    $srcAuo2 = Join-Path $RepoRoot "$Configuration\Win32\x265guiEx.auo"
}
...
$dstAuo2 = Join-Path $PluginDir "x265guiEx.auo2"
Write-Host "==> copy"
Write-Host "    $srcAuo2"
Write-Host " -> $dstAuo2"
Copy-Item -LiteralPath $srcAuo2 -Destination $dstAuo2 -Force

$srcDir = Split-Path -Parent $srcAuo2
foreach ($name in @("x265guiEx.ini", "x265guiEx.en.ini", "x265guiEx.zh.ini")) {
    $srcIni = Join-Path $srcDir $name
